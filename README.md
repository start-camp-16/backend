...
`ADMIN_PASSWORD` 환경변수를 설정하면 `/admin/courses`에서 저장된 코스를 최신순으로
조회하고 제목으로 검색하거나 강제 삭제할 수 있습니다. 브라우저 기본 인증 창의 사용자명은
`admin`, 비밀번호는 배포 환경에 설정한 `ADMIN_PASSWORD`를 사용합니다. 환경변수를 설정하지
않으면 관리자 화면은 비활성화됩니다.
